...
export async function GET(request: Request) {
  ...
  const response = NextResponse.json({ connected: true, pages: (pagesResult.data || []).map((page: { id: string; name: string; tasks?: string[] }) => ({ id: page.id, name: page.name, tasks: page.tasks || [] })) });
